...
# Status values that mean "closed" across all lab languages
_CLOSED_STATUSES = {'afgesloten', 'closed', 'cerrado', 'fermé', 'chiuso',
                    'abgeschlossen', 'fechado', 'stängd', 'مغلق', 'fermé'}
...
def _lab_name_from_source(source) -> str:
    ...
    try:
        ...
        stem = Path(name).stem  # export_CAPA_Italy
        parts = stem.split('_')
...
def load_intl_file(source) -> pd.DataFrame:
    """
    Parse one international CAPA .xls file.
    Returns DataFrame with columns: loc, global_region, init_date, close_date, days2close
    Deduplicates by CAPA ID: earliest init_date, latest close_date per ID.
    Only includes records where close_date is valid (closed records only).
    """
    ...
    if not wb:
        return pd.DataFrame(columns=['loc', 'global_region', 'init_date', 'close_date', 'days2close'])

    sst   = _extract_biff8_sst(wb)
    cells = _extract_biff8_cells(wb, sst)

    # Find the main data sheet: has 'Number', 'Date of notification', 'Date closed' in row 0
    main_sheet = None
    main_cells = {}
    ...
    for row in range(1, max_row + 1):
        ...
        init_v  = main_cells.get((row, init_col), '')
        close_v = main_cells.get((row, close_col), '')
        status  = str(main_cells.get((row, status_col), '')).strip()
        if not capa_id or capa_id in ('nan', ''):
            continue
        init_dt  = _parse_intl_date(str(init_v))
        close_dt = _parse_intl_date(str(close_v))
        raw_rows.append({
            'capa_id':  capa_id,
            'init_dt':  init_dt,
            'close_dt': close_dt,
    ...
    # Deduplicate by CAPA ID: earliest init, latest close
    ...
    # Only keep records with a valid close date (closed records drive metrics)
    df_closed = df_dedup[df_dedup['close_dt'].notna()].copy()
    df_closed = df_closed[df_closed['init_dt'].notna()].copy()

    # Filter by closed status — treat anything NOT recognized as closed as open
    # (open records are tracked separately for ov90 but not closed metrics)
    closed_mask = df_dedup['status'].str.lower().isin(_CLOSED_STATUSES)
    df_closed_status = df_dedup[closed_mask & df_dedup['close_dt'].notna() & df_dedup['init_dt'].notna()].copy()

    df_closed_status['loc']          = lab_name
    df_closed_status['global_region'] = global_region
    df_closed_status['init_date']    = df_closed_status['init_dt']
    df_closed_status['close_date']   = df_closed_status['close_dt']
    df_closed_status['days2close']   = (
        df_closed_status['close_date'] - df_closed_status['init_date']
    ).dt.days.clip(lower=0)

    # Open records (no close date OR status not closed)
    open_mask = ~closed_mask | df_dedup['close_dt'].isna()
    df_open   = df_dedup[open_mask & df_dedup['init_dt'].notna()].copy()
    df_open['loc']           = lab_name
    df_open['global_region'] = global_region
    df_open['init_date']     = df_open['init_dt']
    df_open['close_date']    = pd.NaT

    closed_out = df_closed_status[['loc', 'global_region', 'init_date', 'close_date', 'days2close']].copy()
    open_out   = df_open[['loc', 'global_region', 'init_date', 'close_date']].copy()
    open_out['days2close'] = np.nan
...
def load_and_compute_global(nam_sources, intl_sources, exclude_jn=True):
    """
    Compute unified global metrics combining NAM (multi-year xlsx) + international (xls) files.

    Parameters
    ----------
    nam_sources  : list of file-like objects — NAM yearly xlsx files
    intl_sources : list of file-like objects — international xls files
    exclude_jn   : bool — exclude JN PTOs from NAM data

    Returns
    -------
    dict with all standard D keys PLUS:
      - intl_locations : list of international lab names
      - intl_region_map: {global_region: [lab_names]}
      - global_scope   : 'ALL' | 'NAM' | 'EMEA' | 'APAC'  (set by caller)
      - has_intl       : bool
    """
    # ── Load NAM data (reuse existing pipeline) ────────────────────
    ...
    intl_closed_parts = []
    intl_open_parts   = []
    intl_lab_region   = {}   # lab_name -> global_region
    ...
    for src in intl_sources:
        try:
            ...
            if len(closed_df) > 0:
                lab  = closed_df['loc'].iloc[0]
                grgn = closed_df['global_region'].iloc[0]
                intl_lab_region[lab] = grgn
            elif len(open_df) > 0:
                lab  = open_df['loc'].iloc[0]
                grgn = open_df['global_region'].iloc[0]
                intl_lab_region[lab] = grgn
        except Exception as e:
            pass  # skip bad files silently

    intl_closed = pd.concat(intl_closed_parts, ignore_index=True) if intl_closed_parts else pd.DataFrame(
        columns=['loc', 'global_region', 'init_date', 'close_date', 'days2close'])
    intl_open   = pd.concat(intl_open_parts,   ignore_index=True) if intl_open_parts   else pd.DataFrame(
        columns=['loc', 'global_region', 'init_date', 'close_date', 'days2close'])
    ...
    D_nam_months = D_nam['month_labels']   # e.g. ['Jan 2024', ...]

    all_dates = []
    for df in [intl_closed, intl_open]:
        if 'init_date' in df.columns:
            all_dates.append(df['init_date'].dropna())
        if 'close_date' in df.columns:
            all_dates.append(df['close_date'].dropna())
    ...
    # Offset: how many months to pad at the start of NAM data
    nam_offset = len(months_ext) - len(D_nam_months)

    # ── Rebuild NAM metrics with extended timeline ─────────────────
    # Pad each NAM metric list with zero-rows at the front
    def pad_rows(rows, n):
        zero = {k: 0 for k in rows[0]} if rows else {}
        return [dict(zero) for _ in range(n)] + list(rows)

    def pad_wavg(vals, n):
        return [0] * n + list(vals)

    def pad_D_metrics(D, offset):
        """Pad all metric lists in D by offset months at the front."""
        if offset <= 0:
            return D
        for key in ('car_metrics', 'pto_metrics', 'cmb_metrics'):
            for loc_key in D[key]:
                D[key][loc_key] = pad_rows(D[key][loc_key], offset)
        for key in ('car_wavg', 'pto_wavg', 'cmb_wavg'):
            for loc_key in D[key]:
                D[key][loc_key] = pad_wavg(D[key][loc_key], offset)
        # Update index references
        D['last_dec_idx'] = D['last_dec_idx'] + offset if D['last_dec_idx'] is not None else None
        D['prev_dec_idx'] = D['prev_dec_idx'] + offset if D['prev_dec_idx'] is not None else None
        D['month_labels'] = month_labels_ext
        ye_indices = D.get('year_end_indices', {})
        D['year_end_indices'] = {yr: idx + offset for yr, idx in ye_indices.items()}
        return D

    D_nam = pad_D_metrics(D_nam, nam_offset)
    NM    = len(months_ext)

    # ── Compute intl metrics month by month ───────────────────────
    from calendar import monthrange as _mr
    ...
    def intl_ov90(df_all, lab_keys, m):
        """Count open >90 for a set of locations at month-end m."""
        if df_all is None or len(df_all) == 0:
            return 0
        if lab_keys is not None:
            df = df_all[df_all['loc'].isin(lab_keys)]
        else:
            df = df_all
        me     = _me(m)
    ...
    def intl_calc(closed_df, all_df, lab_keys, months):
        """Compute per-month metrics for a set of lab locations."""
        if lab_keys is not None:
            c   = closed_df[closed_df['loc'].isin(lab_keys)] if len(closed_df) else closed_df
            all_ = all_df[all_df['loc'].isin(lab_keys)]     if len(all_df) else all_df
        else:
            c    = closed_df
            all_ = all_df
        ...
        for m in months:
            ...
            rows.append({'closed': cnt, 'avg_days': avg,
                         'ov90': ov, 'closed_ov90': int((mc['days2close'] > 90).sum()) if cnt > 0 else 0,
                         'total_days': cnt * avg})
        return rows
    ...
    # Build "all" df for intl (closed + open)
    intl_all_parts = []
    if len(intl_closed):
    ...
    intl_all = pd.concat(intl_all_parts, ignore_index=True) if intl_all_parts else pd.DataFrame(
        columns=['loc', 'global_region', 'init_date', 'close_date'])

    # Keys: each lab + each global region + GLOBAL:ALL
    ...
    for lab in intl_labs:
        rows = intl_calc(intl_closed, intl_all, [lab], months_ext)
        intl_car_metrics[lab] = rows
        intl_car_wavg[lab]    = intl_running_wavg(rows)

    # Per global-region metrics
    for grgn, labs in intl_region_map.items():
        rows = intl_calc(intl_closed, intl_all, labs, months_ext)
        intl_car_metrics[f'GLOBAL:{grgn}'] = rows
        intl_car_wavg[f'GLOBAL:{grgn}']    = intl_running_wavg(rows)

    # All intl combined
    rows_all_intl = intl_calc(intl_closed, intl_all, None, months_ext)
    intl_car_metrics['INTL:ALL'] = rows_all_intl
    intl_car_wavg['INTL:ALL']    = intl_running_wavg(rows_all_intl)

    # ── Inject GLOBAL keys into NAM D ─────────────────────────────
    # GLOBAL:NAM = NAM combined (CARs + PTOs) — already in D_nam as 'ALL' in cmb_metrics
    D_nam['car_metrics']['GLOBAL:NAM'] = D_nam['cmb_metrics']['ALL']
    D_nam['car_wavg']['GLOBAL:NAM']    = D_nam['cmb_wavg']['ALL']

    # Merge intl car metrics into D_nam car_metrics
    D_nam['car_metrics'].update(intl_car_metrics)
    D_nam['car_wavg'].update(intl_car_wavg)

    # GLOBAL:ALL = NAM_cmb + INTL combined per month
    nam_cmb_rows  = D_nam['cmb_metrics']['ALL']
    intl_all_rows = intl_car_metrics['INTL:ALL']
    global_all_rows = []
    for i in range(NM):
        n  = nam_cmb_rows[i]
        il = intl_all_rows[i]
    ...
    # GLOBAL:EMEA, GLOBAL:APAC already added above
    # ── Attach intl metadata ──────────────────────────────────────
    D_nam['has_intl']        = True
